...
def Calculator(charge=0, model:str="b973c"):
  return AIMNet2Calculator(load_model(model),charge=charge)

def potential_energy(fragment:ase.Atoms, Calculator)->float:
  """
  Description
  -----------
...
def Optimize(fragment:ase.Atoms, Calculator, fmax:float=0.05, steps=500, clear_log=False)->str:
  """
  Description
  -----------
...
def DIAS(xyzString:str, fragments_params: list | dict, Calculator , clear_log=True)->dict:
  """
  Description
  -----------
  ...
  total_energy = potential_energy(molecule, Calculator=Calculator)
  ...
  for name, fragment in zip(fragment_names, fragments):
    ...
    pre_optimized_Energy = potential_energy(Calculator=Calculator, fragment=frag)
    print(f"\nOptimizer : optimizing {name}")
    Optimize(Calculator=Calculator, fragment=frag, clear_log=False)
    optimized_Energy = potential_energy(Calculator=Calculator, fragment=frag)
...
def trajDIAS(trajFile:str, fragments_params: list | dict, Calculator, resultSavePath:str="./aimDIAS_result.json", save_kws:dict={}):
  """
  Description
  -----------
  ...
  for IRC_idx, xyzString in enumerate(read_traj(trajFile, returnString=True)):
    ...
    trajDIASresult[IRC_idx] = DIAS(xyzString, Calculator=Calculator, fragments_params = fragments_params, clear_log=True)
...
def aimDIAS_run(trajFile:str, fragments_params: list | dict, Calculator, mode:str="calculation", DIAresultPath:str="./aimDIAS_result.json", 
                axis_type:str="irc", geo_param:dict | None =None, unit:str="kcal/mol", include_fragments=False, 
                relative_idx:None|str|int=0, plot_highlights:dict={"marker": False, "linestyle": True}, resultSavePath:str="./aimDIAS_result.json", 
                horizontal_line:bool=True, save_kws:dict={})->None:
  """
  Description
  -----------
  ...
  if mode == "calculation":
    ...
    # run DIAS
    DIASresult = trajDIAS(Calculator=Calculator, trajFile=trajFile, fragments_params=fragments_params, resultSavePath=resultSavePath, save_kws=save_kws)
